...
export default function FireSafety() {
  ...
  return (
    <div className="animate-fade-in">
      {/* Hero */}
      <section id="home" className="section-padding-lg bg-gradient-to-br from-fire-50 via-white to-orange-50 relative overflow-hidden" ref={parallaxRef}>
        <div className="absolute top-10 left-20 w-72 h-72 bg-fire-200/15 rounded-full blur-3xl animate-float" style={{ transform: `translate(${offset.x * 0.5}px, ${offset.y * 0.5}px)` }} />
        <div className="absolute bottom-10 right-10 w-80 h-80 bg-orange-200/15 rounded-full blur-3xl animate-float-slow" style={{ transform: `translate(${offset.x * -0.3}px, ${offset.y * -0.3}px)` }} />
        <div className="absolute top-1/3 left-1/4 w-4 h-4 bg-fire-400/30 rounded-full animate-pulse-soft" />
        <div className="absolute bottom-1/3 right-1/3 w-3 h-3 bg-orange-300/30 rounded-full animate-float-delayed" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="reveal">
                <p className="text-fire-700 text-sm font-medium tracking-wide mb-3 uppercase">DBI Fire & Industrial Safety</p>
              ...
              <div className="reveal space-y-4 text-gray-600 leading-relaxed mb-8" style={{ transitionDelay: '0.15s' }}>
                ...
                <p>DBI's Fire & Industrial Safety division combines decades of on-ground experience with modern automation to build safety infrastructure that organizations can rely on — consistently and confidently.</p>
              </div>
            ...
            <div className="hidden lg:block reveal-right">
              <TiltCard max={8} scale={1.03}>
                <div className="rounded-2xl overflow-hidden shadow-lg border border-fire-100 glow-fire img-zoom relative">
                  <img src={px(2030190, 800)} alt="Firefighter in action with fire hose" className="w-full h-[480px] object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-fire-900/30 to-transparent" />
                </div>
              </TiltCard>
            </div>
          </div>
      ...
      <div className="section-padding !py-0 -mt-8 relative z-10">
        <div className="max-w-7xl mx-auto"><Stats items={stats} variant="fire" /></div>
      </div>

      {/* Marquee */}
      <div className="bg-white border-y border-gray-100">
        <div className="max-w-7xl mx-auto"><Marquee items={marqueeItems} variant="fire" speed={35} /></div>
      </div>
      ...
      <Section id="about" headline="Rooted in experience. Evolved for the future." bg="white" accentColor="fire">
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 mb-8">
          <div className="lg:col-span-2">
            <TiltCard max={6}>
              <div className="rounded-xl overflow-hidden border border-fire-100 img-zoom glow-fire-hover transition-all duration-500">
                <img src={px(4805958, 600)} alt="Fire safety equipment and extinguisher" className="w-full h-full object-cover min-h-[300px]" />
              </div>
            </TiltCard>
          </div>
          <div className="lg:col-span-3">
            <p className="text-gray-600 leading-relaxed mb-4">DBI's Fire & Industrial Safety division has evolved from the legacy of S.B Fire & Safety — carrying forward years of on-ground expertise in fire protection, industrial safety, and regulatory compliance.</p>
            <p className="text-gray-600 leading-relaxed mb-6">Aligned with the Atmanirbhar Bharat mission, we focus on indigenous safety solutions that meet national standards while incorporating modern automation and monitoring technologies.</p>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <GradientBorderCard variant="fire">
                <div className="p-5">
                  <h4 className="font-semibold text-gray-900 text-sm mb-1">Government Association</h4>
                  <p className="text-gray-600 text-sm">Registered and compliant with national fire safety regulatory bodies. Documentation available upon request.</p>
                </div>
              </GradientBorderCard>
              <GradientBorderCard variant="fire">
                <div className="p-5">
                  <h4 className="font-semibold text-gray-900 text-sm mb-1">Certifications</h4>
                  <p className="text-gray-600 text-sm">Team certified in fire safety engineering, industrial risk assessment, and safety system design per national standards.</p>
                </div>
              </GradientBorderCard>
            </div>
          </div>
      ...
      <ParallaxBanner image={px(257636, 1400)} alt="Fire and industrial safety operations" overlay="bg-gradient-to-r from-gray-900/80 via-gray-900/70 to-fire-900/60">
        <div className="text-center text-white px-6">
          <h3 className="text-2xl md:text-3xl font-bold mb-3">Protection that never takes a day off.</h3>
          <p className="text-gray-300 max-w-2xl mx-auto">Serving manufacturing, warehousing, healthcare, education, and government facilities across India.</p>
        </div>
      </ParallaxBanner>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {automationFeatures.map((feature, i) => (
            <TiltCard key={feature.title} max={4}>
              <div className="group bg-fire-50 rounded-xl border border-fire-100 card-hover img-zoom overflow-hidden h-full">
                <img src={feature.image} alt={feature.title} className="w-full h-52 object-cover border-b border-fire-100" />
                <div className="p-6">
                  <div className="flex items-start gap-4">
                    <span className="text-fire-700 font-bold text-lg mt-0.5 shrink-0">{String(i + 1).padStart(2, '0')}</span>
                    <div>
                      <h3 className="font-semibold text-gray-900 mb-2 group-hover:text-fire-700 transition-colors">{feature.title}</h3>
      ...
      <Section id="contact" headline="Plan for safety today." subtitle="Whether you need an audit, a consultation, or a complete safety system design — reach out and we'll respond promptly." bg="gray" accentColor="fire">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 mb-8">
          <TiltCard max={8}>
            <a href="mailto:safety@dbi.co.in" className="block bg-white rounded-xl p-6 border border-gray-100 card-hover text-center group glow-fire-hover transition-all duration-500">
              <div className="mb-3 flex justify-center"><svg className="w-6 h-6 text-gray-400 group-hover:text-fire-600 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" /></svg></div>
              <div className="text-sm font-medium text-gray-900 group-hover:text-fire-700 transition-colors">Email</div>
              <div className="text-xs text-gray-500 mt-1">safety@dbi.co.in</div>
            </a>
          </TiltCard>
          <TiltCard max={8}>
            <a href="tel:+919999999999" className="block bg-white rounded-xl p-6 border border-gray-100 card-hover text-center group glow-fire-hover transition-all duration-500">
              <div className="mb-3 flex justify-center"><svg className="w-6 h-6 text-gray-400 group-hover:text-fire-600 transition-colors" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M2.25 6.75c0 8.284 6.716 15 15 15h2.25a2.25 2.25 0 002.25-2.25v-1.372c0-.516-.351-.966-.852-1.091l-4.423-1.106c-.44-.11-.902.055-1.173.417l-.97 1.293c-.282.376-.769.542-1.21.38a12.035 12.035 0 01-7.143-7.143c-.162-.441.004-.928.38-1.21l1.293-.97c.363-.271.527-.734.417-1.173L6.963 3.102a1.125 1.125 0 00-1.091-.852H4.5A2.25 2.25 0 002.25 4.5v2.25z" /></svg></div>
              <div className="text-sm font-medium text-gray-900 group-hover:text-fire-700 transition-colors">Phone</div>
              <div className="text-xs text-gray-500 mt-1">+91 99999 99999</div>
            </a>
          </TiltCard>
          <TiltCard max={8}>
            <a href="https://wa.me/919999999999" target="_blank" rel="noopener noreferrer" className="block bg-white rounded-xl p-6 border border-gray-100 card-hover text-center group glow-fire-hover transition-all duration-500">
              ...
              <div className="text-sm font-medium text-gray-900 group-hover:text-fire-700 transition-colors">WhatsApp</div>
